...
1. Организатор публикует турнир в **Tournament Service**;
...
1. **UC-1: Организация матча и сбор состава.**
   ...
   **Предусловия:**
      ...
      * Владелец площадки создал расписание с достумными слотами;
...
2. **UC-2: Персонализированное предложение и покупка.**
   ...
   * **Ads Service** сопоставляет обновленные данные профиля с активными рекламными кампаниями;
   * Мобильное приложение запрашивает актуальный рекламный оффер у **Ads Service**;
   * **Ads Service** возвращает персонализированный баннер с Deep Link;
   * При клике на баннер Система инициирует генерацию SSO-токена в **Identity Service**;
   * Пользователь перенаправляется на внешний ресурс с автоматической авторизацией по токену.
