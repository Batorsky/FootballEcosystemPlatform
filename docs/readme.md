...
* **Автономность**: *Мобильное приложение должно поддерживать сбор и локальное хранение данных, в том числе от носимых устройств (через Health-сервисы) в отсутствие интернет-соединения, с последующей автоматической синхронизацией при восстановлении связи*
...
* **Сбор данных:** Мобильный клиент выступает в роли шлюза (Gateway). Данные о тренировках с фитнес-браслетов поступают в локальное хранилище приложения (SQLite) через системные API (HealthKit/Google Fit) по Bluetooth, даже в режиме "Offline";
* **Локальная очередь:** Для передачи данных в Profile Service используется паттерн "Локальная очередь". Каждое событие (завершение тренировки, показатели пульса) кэшируется локально с меткой времени (Timestamp);
* **Синхронизация:** При восстановлении HTTP-соединения мобильное приложение инициирует фоновую выгрузку накопленных данных. Чтобы не перегружать Profile Service, используется пакетная передача (Batching);
* **Идемпотентность:** На стороне бэкенда реализована проверка ID тренировок для исключения дублирования данных при повторных попытках синхронизации.
...
* Consistency vs Availability: В домене бронирования выбран приоритет согласованности (Consistency) через Pessimistic Locking. В социальном домене выбрана модель Eventual Consistency через Kafka;
