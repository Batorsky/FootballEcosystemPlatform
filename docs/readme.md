...
*   **Конкурентный доступ:** Для предотвращения Double Booking в домене *Booking* реализован механизм **Pessimistic Locking** на уровне базы данных.
...
**Действие:** 
1. Игрок обновляет данные в профиле о своих спортивных интересах, инвентаре через **Profile Service**;
...
**Этап А: Анонс и сбор заявок**

1. Организатор создает турнир в **Tournament Service**;
...
3. **Notification Service** подписан на топик `tournaments` и слушает событие tournamentCreated, получает это событие, делает выборку целевой аудитории и отправляет им Push-уведомление;
4. Капитан, кликнув на уведомление, переходит в приложение (Deep Linking на конкнетную страницу турнира);
5. Капитан подает заявку, **Team Service* проверяет состав (ростер) и публикует событие tournamentApplicationSubmitted;
6. Организатор в **Tournament Service** одобряет/отклоняет заявку. Сервис публикует applicationStatusChanged;
7. Notification Service ловит это событие и отправляет капитану PUSH: «Ваша заявка принята» или «Отклонена».

**Этап Б: Формирование сетки и автоматическое бронирование**

1. Организатор формирует сетку. Для каждой пары участников **Tournament Service** генерирует событие `matchScheduled` (с параметрами: наименование и адрес площадки, дата и время матча);
2. **Booking Service** (подписан на `matchScheduled`) автоматически ищет подходящий слот. Если слот найден: резервирует его и публикует событие `slotReserved` (в контексте идентификатор турнира, матча)
3. **Tournament Service**, получив событие slotReserved, фиксирует место в сетке и публикует финальное событие matchConfirmed;
4. По завершении матчей организатор вносит счет; **Tournament Service** автоматически обновляет сетку;
5. Система через **Kafka** и **Notification Service** уведомляет следующих участников о времени и месте их матча.
...
* Operational Complexity: Сложность управления 7+ базами данных, брокером сообщений и K8s.
