...
#### ❗Улучшение существующего архитектурного решения
Для соответствия требованиям по безопасности данных (GDPR/ФЗ-152) и обеспечения гибкости при подключении новых носимых устройств, в концептуальную архитектуру внесены следующие изменения:

**1. Внедрение сервиса Health Adapter**
   Ранее планировавшаяся прямая интеграция мобильного приложения с `Profile Service` заменена на событийно-ориентированную модель через специализированный адаптер.
   * **Что добавилось:** Новый микросервис `Health Adapter`.
   * **Изменение потока данных:** Теперь данные от Apple Health / Google Fit поступают в `Health Adapter`, который нормализует их и публикует анонимизированные события в **Apache Kafka**. `Profile Service` и `Ads Service` становятся подписчиками этих событий.
   * **Результат:** Снижение связности (coupling) и возможность добавлять новые типы устройств (Garmin, Xiaomi) без изменения логики профиля пользователя.

**2. Физическое разделение персональных и метрических данных**
   Для минимизации рисков утечки конфиденциальной информации реализован принцип **Data Segregation**.
   * **Персональные данные (PII):** ФИО, email, телефон и платежные реквизиты хранятся и обрабатываются исключительно внутри контура `Profile Service`.
   * **Анонимизированные метрики:** Данные о физической активности (пульс, шаги, геолокация тренировок) передаются в шину событий и хранилища аналитики под суррогатными идентификаторами (UUID), которые напрямую не связаны с личностью пользователя.
   * **Связывание данных:** Сопоставление метрик с конкретным пользователем происходит только на уровне UI мобильного приложения или при финальной агрегации для отображения в профиле, что исключает наличие полной базы "личность + медицинские показатели" в открытых контурах аналитики.

**3. Модернизация схемы контейнеров (C4 Level 2)**
   На уровне контейнеров вносятся следующие корректировки:
   1. **Health Adapter:** Выделен как самостоятельный Container.
   2. **Kafka Topics:** Выделены отдельные топики для сырых анонимных данных (`health.raw`) и агрегированных показателей (`health.stats`).
   3. **Analytics Store:** (Опционально) Добавлено специализированное хранилище для временных рядов (Time-series DB) для эффективной обработки большого объема метрик с браслетов вне основной транзакционной БД профилей.
...
**Health Adapter Service (Сервис интеграции с носимыми устройствами)**
**Назначение:** Изолированный сервис-адаптер для приема, нормализации и первичной обработки данных физической активности из внешних экосистем (Apple HealthKit, Google Fit, Garmin).

**Основные функции:**
*   **Ingestion:** Прием пакетов данных от мобильных клиентов (в т.ч. накопленных в офлайн-режиме);
*   **Normalization:** Приведение разнородных метрик (шаги, пульс, калории) к единому внутреннему формату платформы;
*   **Anonymization:** Очистка входящего потока от персональных данных (PII). Сервис оперирует анонимными техническими ID, отделяя физические показатели от профиля пользователя;
*   **Streaming:** Публикация анонимизированных метрик в шину событий **Apache Kafka** для дальнейшего потребления сервисами-подписчиками (например, `Profile Service` или `Ads Service`).

**Связи:**
*   **Вход:** Мобильное приложение (через API Gateway).
*   **Выход:** Apache Kafka (топики `health.metrics`).
